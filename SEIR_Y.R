...
covid_Nmif <-        switch(run_level, 10, 200, 100)
...
params_guess = c(
  b0=0.15, alpha=1.07, iota=4.27,
  sigmaSE=1.18,
  br=7.68,
  mu_EI=.16, mu_IR=0.13, # state transition
  rho_V=150, sd_V=3500,                    # measurement V
  rho_Y=.14, od_Y=10,                   # measurement Y
  eta=.05, N=50000,                   # initial value parameters
  S_0=.95, E_0=.04, I_0=.01, R_0=.0)


y = covidSEIRsR %>%
  simulate(params=params_guess, nsim=1000, format="data.frame")
...
foreach(i=1:10,.combine=c) %dopar% {
  library(pomp)
  covidSEIRsR %>% pfilter(params=params_guess,Np=1000)
} -> pf
...
bake(file=paste(cache_address,"/local_search_simple_Y.rds", sep = ''),{
    ...
    covidSEIRsR %>%
      mif2(
        ...
        rw.sd=rw.sd(b0=0.02, alpha=0.02, iota=0.02,
                    sigmaSE=0.02, br=0.02, mu_EI=0.00, mu_IR=0.00, 
                    eta=ivp(0.00), rho_V=0.0, rho_Y=0.0, sd_V=0.0, od_Y=0.0)
      ) #%>%
...
bake(file=paste(cache_address,"/lik_local_simple_Y.rds", sep = ''),{
...
fixed_params <- c(N=50000, mu_EI=.16, mu_IR=0.13, rho_Y=0.14, S_0=.95, E_0=.04, I_0=.01, R_0=.0, eta=0.05, od_Y=0, rho_V=150, sd_V=3500)

#run global search}
bake(file=paste(cache_address,"/global_search_simple_Y.rds", sep = ''),{
    ...
    mf1 %>%
      mif2(params=c(unlist(guess),fixed_params),
           ...
           rw.sd=rw.sd(b0=0.01, alpha=0.01, iota=0.01,
                       sigmaSE=0.01, br=0.01, mu_EI=0.00, mu_IR=0.00,
